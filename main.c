...
{
#ifdef _WIN32
    // If launched from a terminal, attach to its console so TraceLog
    // output is visible.  No-op when launched from Explorer.
    ...
    while (!WindowShouldClose()) {
        ...
        if (!child_exited) {
#ifdef _WIN32
            // On Windows, the ConPTY pipe may not close when the child
            // process exits.  Check the process handle directly to detect
            // exit independently of the pipe state.
            if (WaitForSingleObject(pty_ctx.process, 0) == WAIT_OBJECT_0)
                child_exited = true;
